  ...
  <div class="relative lg:col-span-6 border border-border rounded-lg py-3 px-1">
    ...
    <div v-if="isLoading" class="absolute inset-0 bg-black/50 z-10 flex items-center justify-center rounded-lg">
      <div class="flex flex-col items-center">
        ...
        <span class="text-white mt-2">Loading data...</span>
      </div>
    </div>
...
watch(() => chartStore.seriesData, (newData) => {
  if (newData) {
...
onMounted(async () => {
  ...
  await chartStore.fetchSeriesData();
  setChartData();
});
...
const setChartData = () => {
  ...
  try {
    ...
    const seriesData = chartStore.seriesData;
    ...
    const regions = Object.keys(seriesData);
    const airtelData = [];
    const mtnData = [];

    regions.forEach(region => {
      airtelData.push(seriesData[region].airtel);
      mtnData.push(seriesData[region].mtn);
    });
